...
def tile_floating_windows(workspace_id):
    ...
    if not windows:
        print("No floating windows in the active workspace.")
        return False
...
def float_all_tiled(workspace_id):
    ...
    if not tiled:
        print("No tiled windows in the active workspace.")
        return False

    print(f"Floating {len(tiled)} windows...")
...
def main():
    ...
    try:
        ...
        if workspace_id is None:
            print("Error: could not get the active workspace.")
            sys.exit(1)
