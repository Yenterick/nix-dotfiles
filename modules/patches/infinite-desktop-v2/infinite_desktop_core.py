...
def classify_device(path):
    ...
    abs_codes = set(c[0] if isinstance(c, tuple) else c for c in caps.get(ecodes.EV_ABS, []))
    ...
    is_touchpad = (
        (ecodes.ABS_X in abs_codes or ecodes.ABS_MT_POSITION_X in abs_codes)
        and (ecodes.ABS_Y in abs_codes or ecodes.ABS_MT_POSITION_Y in abs_codes)
            ...
            if etype == EV_KEY and code == BTN_LEFT:
                ...
                if is_x:
                    if abs_last_x is not None:
                        delta = value - abs_last_x
                        mouse_rel_x += delta
                        if super_pressed and alt_pressed:
                            sign = -1 if read_inverted() else 1
                            acc_x += delta * speed * ABS_SPEED_SCALE * sign
                        else:
                            acc_x = 0.0
                    abs_last_x = value
                else:
                    if abs_last_y is not None:
                        delta = value - abs_last_y
                        mouse_rel_y += delta
                        if super_pressed and alt_pressed:
                            sign = -1 if read_inverted() else 1
                            acc_y += delta * speed * ABS_SPEED_SCALE * sign
                        else:
                            acc_y = 0.0
                    abs_last_y = value
        ...
        try:
            ...
            for path in touchpads:
                ...
                if t is None or not t.is_alive():
                    ...
                    print(f"[+] Touchpad detected: {path}", flush=True)
        except Exception as e:
