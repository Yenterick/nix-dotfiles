...
EV_KEY=1; EV_REL=2; EV_ABS=3; REL_X=0; REL_Y=1
...
# Touchpad (absolute-coordinate) tracking. On most laptop touchpads the legacy
# "Mouse" evdev node advertises relative-motion capability but never actually
# emits REL events -- real motion (and often clicks) only arrive as EV_ABS on
# the "Touchpad" node. When that's what mouse_dev points at, we synthesize
# relative deltas from consecutive absolute samples instead.
abs_last_x = None
abs_last_y = None
ABS_SPEED_SCALE = 0.35  # touchpad coordinate units are much denser than mouse counts
...
def mouse_reader():
    global acc_x, acc_y, btn_left, abs_last_x, abs_last_y
    ...
    while True:
        ...
        with lock:
            if etype == EV_KEY and code == BTN_LEFT:
                btn_left = (value == 1)
            elif etype == EV_KEY and code in (BTN_TOOL_FINGER, BTN_TOUCH) and value == 0:
                # Finger lifted: drop the absolute baseline so the next touch-down
                # doesn't register as a huge jump.
                abs_last_x = None
                abs_last_y = None
            elif etype == EV_REL:
                if super_pressed and alt_pressed and btn_left:
                    ...
                    if not (super_pressed and alt_pressed and btn_left):
                        acc_x = 0.0
                        acc_y = 0.0
            elif etype == EV_ABS and code in (ABS_X, ABS_MT_POSITION_X, ABS_Y, ABS_MT_POSITION_Y):
                is_x = code in (ABS_X, ABS_MT_POSITION_X)
                if super_pressed and alt_pressed and btn_left:
                    sign = -1 if read_inverted() else 1
                    if is_x:
                        if abs_last_x is not None:
                            acc_x += (value - abs_last_x) * speed * ABS_SPEED_SCALE * sign
                        abs_last_x = value
                    else:
                        if abs_last_y is not None:
                            acc_y += (value - abs_last_y) * speed * ABS_SPEED_SCALE * sign
                        abs_last_y = value
                else:
                    # Keep tracking position so we have a fresh baseline the
                    # moment the modifier+button combo becomes active.
                    if is_x:
                        abs_last_x = value
                    else:
                        abs_last_y = value
